...
def main_demo(args):
    ...
    depth = cv2.imread(args.depth, cv2.IMREAD_UNCHANGED).astype(np.float32)
    resized = cv2.resize(depth, (W, H), interpolation = cv2.INTER_LINEAR)
    resized = resized * -1
    ...
    mask1 = np.random.rand(H, W) > 0
    points = rays[mask1][:,:3] + rays[mask1][:,3:]*resized.reshape((-1,1))
    pcd = o3d.t.geometry.PointCloud(points)
    obj = np.asarray(pcd.to_legacy().points)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image', default='demo/web/gudetama/im.png', type=str, help='input image file')
    parser.add_argument('--depth', default='demo/web/gudetama/depth.png', type=str, help='input depth file')
    parser.add_argument('--seg', default='demo/web/gudetama/seg.png', type=str, help='input seg file')
    ...
    parser.add_argument('--fov', default=35, type=int, help='Field of view for depth to point cloud')
